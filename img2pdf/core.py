...
def fld2pdf(folder: Path, out: str):
    banner_file = folder / "fristbanner.jpg"
    
    files = [file for file in folder.glob(r'*') if re.match(r'.*\.(jpg|png|jpeg|webp)', file.name)]#, re.IGNORECASE)]
    
    files.sort(key=lambda x: x.name)
    
    if banner_file.exists() and banner_file in files:
        files.remove(banner_file)
        files.insert(0, banner_file)
