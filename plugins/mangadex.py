...
class MangaDexClient(MangaClient):
    ...
    languages = ("en",)
    language_param = '&'.join([f'translatedLanguage[]={lang}' for lang in languages])
    name = f'{name}-{languages[0]}'
    ...
    def mangas_from_page(self, page: bytes):
        ...
        mangas = [MangaDexMangaCard(self, *tup) for tup in zip(names, url, images, ids, "None")]
    ...
    def chapters_from_page(self, page: bytes, manga: MangaCard = None):
        ...
        return list(map(lambda x: MangaDexMangaChapter(self, x[0], x[1], manga, [], x[2], "None"), zip(texts, links, ids)))
