...
class ComickClient(MangaClient):
    ...
    def chapters_from_page(self, datas: bytes, slug: str, manga: MangaCard = None):
        ...
        for data in datas["chapters"]:
            ...
            texts.append(text)
            chapters.append(MangaChapter(self, text, link, manga, [], "None"))
        return chapters
    ...
    async def pictures_from_chapters(self, data, response=None):
    ...
    async def iter_chapters(self, manga_url: str, manga_name) -> AsyncIterable[MangaChapter]:
        ...
        slug = request_url.split("/")[4]
        if slug in hid_query:
            hid = hid_query[slug]
        else:
            hid = await ComickClient().get_hid(slug)
        
        datas = await ComickClient().get_comics(hid=hid, page=1)

        for chapter in self.chapters_from_page(datas, slug, manga_card):
            yield chapter
